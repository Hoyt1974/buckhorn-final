      ...
      <div className="mx-auto max-w-6xl px-4 h-16 flex items-center justify-between">
        ...
        <nav className={clsx("items-center gap-6 hidden md:flex")}>
          ...
          <div className="relative">
            <button onClick={()=>setShowCrypto(v=>!v)} className="px-3 py-1.5 rounded-full border border-line hover:border-accent/60">{labels.nav.crypto} ▾</button>
            {showCrypto && (
          ...
          <div className="relative">
            <button onClick={()=>{ /* noop - languages shown below */ }} className="px-3 py-1.5 rounded-full border border-line hover:border-accent/60">{labels.nav.language} ▾</button>
            <div className="absolute right-0 mt-2 bg-card border border-line rounded-lg shadow-soft min-w-44">
              {config.languages.map((l:any) => (
                <button key={l.code} onClick={()=> setLang(l.code)} className="block w-full text-left px-3 py-2 hover:bg-white/5">{l.label}</button>
              ))}
            </div>
          </div>
        </nav>
